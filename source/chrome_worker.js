  ...
  browser.storage.local.get(
    {
      ...
      myPrivateKey: null,
      passphrase: null,
      contextMenus: [false, false, false],
    },
    async function (result) {
      ...
      const {
        keys: [privateKey],
      } = await openpgp.key.readArmored(myPrivateKey);
      ...
      if (result.contextMenus[0]) message = decrypted;
...
browser.runtime.onConnect.addListener(function (port) {
  if (message != null) port.postMessage({ message: message });

  port.onMessage.addListener(function (msg) {
    if (msg.seen) message = null;
  });
